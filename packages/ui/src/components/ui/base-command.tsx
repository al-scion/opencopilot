...
} from "@packages/ui/components/ui/autocomplete";
...
function CommandInput({
	...
	wrapperClassName,
	...props
}: React.ComponentProps<typeof AutocompleteInput> & { wrapperClassName?: string }) {
	return (
		<div className={cn("flex items-center px-2.5 py-1.5", wrapperClassName)}>
			<AutocompleteInput
				autoFocus
				className={cn(
					"border-transparent! bg-transparent! shadow-none before:hidden has-focus-visible:ring-0",
					"font-normal text-sm leading-9",
					className
				)}
...
function CommandShortcut({ className, ...props }: React.ComponentProps<"kbd">) {
	return (
		<kbd
			className={cn("ms-auto font-light text-muted-foreground text-xs tracking-wide", className)}
